...
class agent():
   ...
   #  	    self.time_step += 1
	  ...
	  #   minibatch = random.sample(tmp_deque, BATCH_SIZE) state_batch =
	  #   [data[0] for data in minibatch] action_batch = [data[1] for
	  #   data in minibatch] reward_batch = [data[2] for data in
	  #   minibatch] next_state_batch = [data[3] for data in minibatch]
...
import class_move_gym

# step 1: connect the gym
# import gym
env = class_move_gym.move_gym()
